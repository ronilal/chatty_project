...
class MyGui:
    def __init__(self, sock):
        ...
        self.Ebutton = tk.Button(self.window, text='Exit', command=self.Exit_srv)
        self.Ebutton.pack(padx=20, pady=5)
            ...
            try:
                ...
                if message:
                    ...
                    if user_data['code'] == 'sending':
                        ...
                        self.image_array = np.array(self.sent_image)
    ...
    def home_page(self):
        ...
        Exit_button = tk.Button(self.home, text="Exit", command=self.Exit_srv)
        Exit_button.pack()
    ...
    def Exit_srv(self):
        self.con = False
        self.sock.close()
        self.windowBye = tk.Tk()

        self.labelBye = tk.Label(self.windowBye, text="you have been disconnected, please close all related windows...")
        self.labelBye.pack(padx=20, pady=5)

        self.windowBye.mainloop()
    ...
    def ask_image_page(self,sender):
        ...
        label = tk.Label(self.ask_image, text = sender + " sent a picture, do you want to sace it?")
        label.pack()

        button = tk.Button(self.ask_image, text="yes", command=self.save_sent_pic)
        button.pack()
    ...
    def save_sent_pic(self):
        ...
        label = tk.Label(self.image_page2, text="image seccesfuly saved!")
        label.pack()
    ...
    def sendInfoChange(self):
        data = self.newpass_entry.get()
        send_with_size(self.sock,data)
    ...
    def SignUp(self):
        if self.CheckConPass():
            ...
            if self.validatemail(email)==True:
                print("unvalid email")
                close_window(self.Swindow)
                self.SignUpMsg()
                return
